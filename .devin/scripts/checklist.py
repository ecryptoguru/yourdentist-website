...
Usage:
    python .devin/scripts/checklist.py .                    # Run core checks
    python .devin/scripts/checklist.py . --url <URL>        # Include performance checks
...
CORE_CHECKS = [
    ("Security Scan", ".devin/skills/vulnerability-scanner/scripts/security_scan.py", True),
    ("Lint Check", ".devin/skills/lint-and-validate/scripts/lint_runner.py", True),
    ("Schema Validation", ".devin/skills/database-design/scripts/schema_validator.py", False),
    ("Test Runner", ".devin/skills/testing-patterns/scripts/test_runner.py", False),
    ("UX Audit", ".devin/skills/frontend-design/scripts/ux_audit.py", False),
    ("SEO Check", ".devin/skills/seo-fundamentals/scripts/seo_checker.py", False),
]

PERFORMANCE_CHECKS = [
    ("Lighthouse Audit", ".devin/skills/performance-profiling/scripts/lighthouse_audit.py", True),
    ("Playwright E2E", ".devin/skills/webapp-testing/scripts/playwright_runner.py", False),
]
...
def print_summary(results: List[dict]):
    ...
    required_failed_count = sum(1 for r in results if not r["passed"] and not r.get("skipped") and r.get("required", False))
    advisory_failed_count = sum(1 for r in results if not r["passed"] and not r.get("skipped") and not r.get("required", False))
    ...
    if advisory_failed_count > 0:
        print(f"{Colors.YELLOW}ℹ️  Advisory failures: {advisory_failed_count}{Colors.ENDC}")
    print()
    ...
    if required_failed_count > 0:
        print_error(f"{required_failed_count} required check(s) FAILED - Please fix before proceeding")
        return False
    if advisory_failed_count > 0:
        print_warning(f"{advisory_failed_count} advisory check(s) need follow-up")
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python .devin/scripts/checklist.py .                      # Core checks only
  python .devin/scripts/checklist.py . --url http://localhost:3000  # Include performance
        """
    )

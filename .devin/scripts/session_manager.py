...
Usage:
    python .devin/scripts/session_manager.py status [path]
    python .devin/scripts/session_manager.py info [path]
"""
...
def analyze_package_json(root: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        if "svelte" in all_deps: stack.append("Svelte")
        if "express" in all_deps: stack.append("Express")
        if "nestjs" in all_deps or "@nestjs/core" in all_deps: stack.append("NestJS")
...
def count_files(root: Path) -> Dict[str, int]:
    stats = {"total": 0}
    exclude = {".git", "node_modules", ".next", "dist", "build", ".devin", ".gemini", "__pycache__"}

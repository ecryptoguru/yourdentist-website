...
1. **Load memory-system skill** — Read `.devin/skills/memory-system/SKILL.md` first
...
- Memory location: .devin/memory/

WORKFLOW:
1. CLASSIFY the information type: user | feedback | project | reference
2. CHECK if relevant topic file exists in .devin/memory/
3. SAVE to appropriate topic file (create if needed)
4. UPDATE .devin/memory/MEMORY.md index with one-line pointer
...
File: .devin/memory/[topic-file].md
